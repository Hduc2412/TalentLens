...
export function EmployeeCard({ person, departments, canEdit, showSensitive, onSelect, onMove, onDragStart }) {
  ...
  return <article className={`person-card tone-${tone}`} draggable={canEdit} onDragStart={() => onDragStart(person.id)}>
    <button type="button" className="person-card-open" onClick={() => onSelect(person.id)} aria-label={`Xem hồ sơ ${person.id} ${person.name}`}>
      ...
      <div className="trait-row">{person.traits.map((trait) => <span key={trait}>{trait}</span>)}{showSensitive && person.sensitiveTraits.map((trait) => <span className="sensitive-trait" key={trait}>{trait}</span>)}</div>
    </button>
