...
export function DepartmentLane({ department, departments, members, canEdit, role, isDropTarget, onDropTargetChange, onDrop, onMove, onSelect, onDragStart }) {
  ...
  return <section className={`department-lane ${isDropTarget ? 'drop-ready' : ''}`}
    ...
    <div className="lane-content">
      {members.map((person) => <EmployeeCard key={person.id} person={person} departments={departments} canEdit={canEdit} showSensitive={role === 'hr'} onSelect={onSelect} onMove={onMove} onDragStart={onDragStart} />)}
